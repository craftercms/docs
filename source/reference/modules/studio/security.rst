...
The attribute ``secure_key`` is placed by the authentication agent in the header |br|
...
The optional attribute ``x-crafter-oidc-data`` supports headers coming in JSON web tokens (JWT) format (generated by a 3rd party). Note that if this token is available and contains the credentials (username, email, security_key, etc...), the detailed credentials parsed from the token claims will be used for the authentication flow.
