...
* Requests with the preview token query string argument (QSA) now sets the site ID and preview token as cookies in the URL.
  This allows subsequent requests to automatically reuse the token and site context without needing to resend them in the URL. |br|
