...
To view your site with respect to the staging preview, we need to set it up for delivery.  To setup your site for the staging preview, run the ``init-site`` script and follow the instructions in :ref:`newIa-setup-site-for-delivery` using the staging branch in the options.
...
To sync the staging target with the live target, simply call the ``reset-staging`` API.  See :ref:`newIa-crafter-studio-api-publish-reset-staging` for more details on the API
