...
#. **Download and install required items for creating binaries**
   ...
   * make and GCC
   ...
   .. code-block:: bash
      :caption: *Install prerequisites for creating Crafter CMS binaries*
...
#. Unpack the file (``crafter-cms-authoring-VERSION.tar.gz``) into any directory.  You may now follow the instructions listed :ref:`above<installing-crafter-cms-on-wsl>` starting from the step to ``Extract the Crafter CMS binary`` to start using Crafter CMS
