...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Crafter supports both Linux and Windows based operating systems. All of the concepts in this document apply to both platforms. That said, Linux and Windows handle basic operations like reads, writes, permissions and file locking differently. It’s important to tune specifically for the operating system you are on.
