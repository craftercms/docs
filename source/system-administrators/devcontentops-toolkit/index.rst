...
To run the command line tool (CrafterCMS Command line Interface (CLI) for DevContentOps processes), download the **DevContentOps Toolkit** binary archive from https://craftercms.com/download and extract the contents to any directory.  In your terminal program, navigate to the ``bin`` folder under the directory where you extracted the tool, ``DOWNLOAD_DIR/crafter-cli-VERSION/bin/``.  There you will find two versions of the tool:
